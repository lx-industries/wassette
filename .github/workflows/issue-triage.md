...
2. Retrieve the issue content using the `get_issue` tool. If the issue is obviously spam or something else that is not an actual issue to be worked on, then do nothing and exit the workflow. **Note**: Subtask issues created by workflows (marked with `[subtask]` prefix) are legitimate issues and should be triaged normally.
